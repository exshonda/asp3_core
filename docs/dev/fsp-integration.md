...
### asp3_fsp側 submodule化＋CMake一本化 — EK-RA6M5／EK-RA8M2 ビルド検証 完了（2026-06-11）
...
**EK-RA8M2（Cortex-M85）も同手順で実ビルド検証**：RASC `--generate`→clang ビルドで
`asp3_fsp.elf`（text 34,574）/`.hex` 生成。`sta_ker`/`_kernel_dispatcher_*` と FSP
（`R_BSP_Init_RTC`/`R_IOPORT_Open`）の同一ELFリンク・cfg 3パスを確認。cfg1_out は
M85 向け最小リンク（`-mcpu=cortex-m85 -mfpu=fpv5-d16`）。

**実機（EK-RA8M2）で判明した M85 バグと修正（2026-06-11）**：
EK-RA8M2 実機で起動バナー後に出力が文字化け。原因は asp3_core 共通arch
`core_support.S` の `exc_return_const`（全コンテキストスイッチが `ldr pc, exc_return_const`
で参照）が、`TOPPERS_FPU_CONTEXT` 有効時に直前の 16bit `bx lr` で 2バイト境界に置かれ、
