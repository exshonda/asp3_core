...
（注：2026-06-16 に QEMU の M33 ターゲットを mps2-an505/IoTKit へ置換。FPU が CPU0 に実装されハード浮動小数点で再検証可能になった。下表の「回帰（POSIX／QEMU mps2-an521）」は置換前の記録。以降の M33 QEMU 回帰は mps2-an505 を使用。）
