...
（注：2026-06-16 に QEMU の M33 ターゲットを mps2-an505/IoTKit へ置換〔現ターゲット名 mps2_an505_gcc・プリセット mps2_an505-qemu〕。FPU が CPU0 に実装されハード浮動小数点で再検証可能になった。本ファイル中の `mps2_an521` / `mps2-an521` 表記〔実施プランおよび下記の変更ファイル・検証結果〕は置換前の記録。以降の M33 QEMU 検証は mps2-an505 を使用。）
