...
**残り手順**：
- 手順3：simtimer.txt の削除（file-cleanup追補・DIVERGENCE_MAP記録）
- 手順4：参照の張り替え（AGENTS §13・README・docs内の doc/*.txt 参照→docs/spec/。
  doc/原本の削除はユーザー確認のうえ実施）
- 手順5：記録（本ファイル実施結果の総括・README索引最終化）
