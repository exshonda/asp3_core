...
1. 既存 `asp3_pico_sdk` の `target/rp2350-arm-s_pico_sdk` を新構成へ再配置
   （asp3_core を submodule 化・`ASP3_TARGET_DIR` で外部target指定）
2. `asp3_pico_sdk.cmake`：`PICO_PLATFORM`→`ASP3_TARGET`＋`ASP3_TARGET_DIR` 解決
3. sample1 を単一フォルダ化（PICO_PLATFORMでarm/riscv切替）
4. **タイマ競合の解決**：SDK alarm（TIMER0）と ASP3 HRT（TIMER0）の調停
5. RISC-V版は後続（asp3_pico_sdk未対応のため・本リポジトリの
   `raspberrypi_pico2_riscv_gcc` のXh3irq実装を移植元にできる）
6. 検証は実機（pico-sdk必須・CIはビルドまで）
