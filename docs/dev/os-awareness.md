...
| stm32 実機（移動後の osdebug 回帰） | − | 実機側 PC で実施（開発機に aarch64-none-elf 無し。aarch64-linux-gnu でのリンクは既存の環境制約で不可＝今回の変更と無関係であることを main でも確認） |
| pico2 実機 | − | スコープ＝層の実装まで（osdebug は SWD 用が未定義のため対象外） |
