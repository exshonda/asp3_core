...
（注：2026-06-16 に QEMU ターゲットを mps2-an505/IoTKit へ置換。FPU が CPU0 に実装されハード浮動小数点で再検証可能になった。下表の mps2-an521（M33）と上記の追加ファイル `target/mps2_an521_gcc/target_os_awareness.py` は置換前の記録〔現ターゲット名は mps2_an505_gcc〕。以降の M33 QEMU 検証は mps2-an505 を使用。）
