...
| TTSP3 functional（--only yaml） | ○ | **PASS 1174・FAIL 0・SKIP 639 / 1813**（an505/an547 と一致＝ARMv7-M でも全件適合。2026-06-16 実行。※初回 full run で2件のビルド失敗を観測したが単体PASS＋クリーン再実行で FAIL 0＝並列ビルドのフレーキー） |
