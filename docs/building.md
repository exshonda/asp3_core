...
cmake --build build/mps2_an521_gcc --target run
```
...
```bash
cd build/mps2_an521_gcc
ninja        # ビルド
ninja run    # ビルドして実行

# cd したくない場合
ninja -C build/mps2_an521_gcc run
```

buildPresets（`run-linux_gcc`／`run-mps2_an521_gcc`／`run-zybo_z7_gcc-qemu`／`run-zcu102_arm64_gcc-qemu`）でルートから
`cmake --build --preset run-mps2_an521_gcc` の1コマンド実行もできる。
...
# ターゲット別
cmake --preset mps2_an521_gcc -B build/mps2_an521_gcc
cmake --preset linux_gcc    -B build/linux_gcc

# 同じターゲットで構成違い（例：テストプログラム）
cmake --preset mps2_an521_gcc -B build/mps2-sem1 \
...
- `compile_commands.json` がビルドフォルダに自動生成される
  （`ln -sf build/mps2_an521_gcc/compile_commands.json compile_commands.json` でルートにリンク：AGENTS.md §5）
...
```bash
ninja -C build/mps2_an521_gcc run     # QEMU（終了は Ctrl-A X）
ninja -C build/linux_gcc run        # ホスト実行
ninja -C build/raspberrypi_pico2_gcc run    # 実機書込み（OpenOCD program）
```
...
qemu-system-arm -machine mps2-an521 -nographic \
  -semihosting-config enable=on,target=native -kernel build/mps2_an521_gcc/asp.elf

# zybo（UART1が2本目の-serial）
qemu-system-arm -M xilinx-zynq-a9 -semihosting -nographic \
  -serial /dev/null -serial mon:stdio -kernel build/zybo_z7_gcc-qemu/asp.elf
```
...
(sleep 3; printf 'r'; sleep 3) | timeout 10 qemu-system-arm -machine mps2-an521 \
  -nographic -semihosting-config enable=on,target=native -kernel build/mps2_an521_gcc/asp.elf
```
...
echo '--preset mps2_an521_gcc' > TARGET_OPTIONS
...
**プリセット名はターゲット名と同一**。QEMU/実機の両対応で条件コンパイルが
変わるターゲットは、QEMU側を `<ターゲット名>-qemu` とする。

| ターゲット | プリセット | 実行 |
|---|---|---|
| linux_gcc | linux_gcc | ホスト実行（run） |
| mps2_an521_gcc | mps2_an521_gcc | QEMU（run．QEMU専用ターゲット） |
| zybo_z7_gcc | zybo_z7_gcc／**zybo_z7_gcc-qemu** | 実機（xilinx_sdk/jtag.tcl）／QEMU（run） |
| zcu102_arm64_gcc | zcu102_arm64_gcc／**zcu102_arm64_gcc-qemu** | 実機（実行手段は今後整備）／QEMU（run）．aarch64-none-elf が無い環境は `-DA35_TOOLCHAIN_PREFIX=aarch64-linux-gnu-` を付与 |
| raspberrypi_pico2_gcc | raspberrypi_pico2_gcc | 実機（run=OpenOCD書込み．gdb/console等） |
| stm32mp257f_dk_arm64_gcc | stm32mp257f_dk_arm64_gcc | 実機（swd-run/osdebug/console等．リンクは aarch64-none-elf 環境） |
