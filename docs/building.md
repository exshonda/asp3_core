...
cmake --build build/mps2_an521-qemu --target run
```
...
```bash
cd build/mps2_an521-qemu
ninja        # ビルド
ninja run    # ビルドして実行

# cd したくない場合
ninja -C build/mps2_an521-qemu run
```

buildPresets（`run-linux`／`run-mps2_an521-qemu`／`run-zybo_z7-qemu`／`run-zcu102_arm64-qemu`）でルートから
`cmake --build --preset run-mps2_an521-qemu` の1コマンド実行もできる。
...
cmake --preset mps2_an521-qemu -B build/mps2-sem1 \
...
- `compile_commands.json` がビルドフォルダに自動生成される
  （`ln -sf build/mps2_an521-qemu/compile_commands.json compile_commands.json` でルートにリンク：AGENTS.md §5）
...
qemu-system-arm -machine mps2-an521 -nographic \
  -semihosting-config enable=on,target=native -kernel build/mps2_an521-qemu/asp.elf
...
(sleep 3; printf 'r'; sleep 3) | timeout 10 qemu-system-arm -machine mps2-an521 \
  -nographic -semihosting-config enable=on,target=native -kernel build/mps2_an521-qemu/asp.elf
```
