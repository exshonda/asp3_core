...
cmake --build build/mps2_an521 --target run
```
...
# cd したくない場合
ninja -C build/mps2_an521 run
```

buildPresets（`run-linux`／`run-mps2_an521`／`run-zybo_z7-qemu`／`run-zcu102_arm64-qemu`）でルートから
`cmake --build --preset run-mps2_an521` の1コマンド実行もできる。
...
cmake --preset mps2_an521 -B build/mps2-sem1 \
...
- `compile_commands.json` がビルドフォルダに自動生成される
  （`ln -sf build/mps2_an521/compile_commands.json compile_commands.json` でルートにリンク：AGENTS.md §5）
...
```bash
ninja -C build/mps2_an521 run     # QEMU（終了は Ctrl-A X）
...
qemu-system-arm -machine mps2-an521 -nographic \
  -semihosting-config enable=on,target=native -kernel build/mps2_an521/asp.elf
...
(sleep 3; printf 'r'; sleep 3) | timeout 10 qemu-system-arm -machine mps2-an521 \
  -nographic -semihosting-config enable=on,target=native -kernel build/mps2_an521/asp.elf
```
...
echo '--preset mps2_an521' > TARGET_OPTIONS
...
| mps2_an521_gcc | mps2_an521 | QEMU（run．QEMU専用ターゲット） |
