 ...
 *  の責任を負わない．
 * 
 *  $Id: tBitKernel.c 1881 2026-05-18 03:52:07Z ertl-hiro $
 */
...
bit_mutex(ID mtxid)
{
	...
	if (p_loctsk != NULL) {
		...
		while ((p_queue = p_queue->p_next) != &(p_mtxcb->wait_queue)) {
			 ...
			 *  優先度上限の検査
			 */
			if (MTXPROTO(p_mtxinib) == TA_CEILING) {
...
bit_tmevt(uint_t notmevt)
{
	...
	/*
	 *  LAST_INDEX()（＝tmevt_heap[0].last_index）の検査
	 *  設定されているタイムイベント数（notmevt）の検査
	 */
	if (notmevt != LAST_INDEX()) {
		return(E_SYS_LINENO);
	}
	 ...
	 *  タイムイベントノード毎の検査
	 */
	for (index = ROOT_INDEX; index <= LAST_INDEX(); index++) {
		/*
		 *  対応するタイムイベントブロックを取得
		 */
		p_tmevtb = tmevt_heap[index].p_tmevtb;

		/*
		 *  タイムイベントブロック中のindexの検査
		 */
		if (p_tmevtb->index != index) {
			return(E_SYS_LINENO);
		}

		/*
		 *  タイムイベントブロック中のevttimの検査
		 */
		/* 左の子ノードよりイベント発生時刻が早いか同じ */
		if ((child = LCHILD(index)) <= LAST_INDEX()) {
			if (!EVTTIM_LE(p_tmevtb->evttim,
								tmevt_heap[child].p_tmevtb->evttim)) {
				return(E_SYS_LINENO);
			}
		}
		/* 右の子ノードよりイベント発生時刻が早いか同じ */
		if ((child = RCHILD(index)) <= LAST_INDEX()) {
			if (!EVTTIM_LE(p_tmevtb->evttim,
								tmevt_heap[child].p_tmevtb->evttim)) {
				return(E_SYS_LINENO);
			}
