...
arch/              [EXTENDED]  アーキ依存（arm_m_gcc/=Cortex-M, arm64_gcc/=Cortex-A35/AArch64(chip: stm32mp2), riscv_gcc/=RV64GC(chip: polarfire_soc)）
...
解析・検証：
```bash
./build/linux/asp 2>&1 | python3 scripts/parse_slog.py            # 人間可読
./build/linux/asp 2>&1 | python3 scripts/parse_slog.py --json \
  > actual.jsonl
