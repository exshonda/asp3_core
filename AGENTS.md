...
arch/              [EXTENDED]  アーキ依存（arm_m_gcc/=Cortex-M, arm64_gcc/=Cortex-A35/AArch64(chip: stm32mp2), riscv_gcc/=新規）
...
cmake --preset mps2_an521_gcc -B build/mps2_an521_gcc && cmake --build build/mps2_an521_gcc
timeout 30 qemu-system-arm -M mps2-an521 -cpu cortex-m33 \
  -kernel build/mps2_an521_gcc/asp.elf \
  -semihosting -semihosting-config enable=on,target=native -nographic

# QEMU Cortex-A53（xlnx-zcu102・ハードなし）
cmake --preset zcu102_arm64_gcc-qemu -B build/zcu102_arm64_gcc-qemu && cmake --build build/zcu102_arm64_gcc-qemu
timeout 30 qemu-system-aarch64 -machine xlnx-zcu102,secure=on -nographic \
  -semihosting-config enable=on,target=native -kernel build/zcu102_arm64_gcc-qemu/asp.elf

# Raspberry Pi PICO2 (Cortex-M33 / 実機)
cmake --preset raspberrypi_pico2_gcc -B build/raspberrypi_pico2_gcc && cmake --build build/raspberrypi_pico2_gcc
...
cmake --preset stm32mp257f_dk_arm64_gcc -B build/stm32mp257f_dk_arm64_gcc && cmake --build build/stm32mp257f_dk_arm64_gcc
```
...
> **前提メモ**
> - ビルドは **CMakeのみ**（Makefile版は廃止済み。詳細は `docs/building.md`）。**プリセット名＝ターゲット名**（QEMU/実機両対応のターゲットはQEMU側を `<ターゲット名>-qemu`）。一覧は `cmake --list-presets`。
...
```bash
ln -sf build/linux_gcc/compile_commands.json compile_commands.json
```
...
CIで `clang-tidy` / `cppcheck` がチェックする。生成コードがこれに違反しないこと。
...
解析・検証：
```bash
./build/linux_gcc/asp 2>&1 | python3 scripts/parse_slog.py            # 人間可読
./build/linux_gcc/asp 2>&1 | python3 scripts/parse_slog.py --json \
  > actual.jsonl
...
例: feat(target): add mps2-an521 QEMU target
...
7. 変更後、POSIX → QEMU(mps2-an521) の順で回帰テストを実行する。
...
| リポジトリ | 参照目的 |
|---|---|
| [asp3_pico_sdk](https://github.com/exshonda/asp3_pico_sdk) | RP2350ターゲット・CMake実装のベース |
| [asp3_fsp](https://github.com/exshonda/asp3_fsp) | Renesas FSP統合 |
| [stm32_vscode_asp](https://github.com/exshonda/stm32_vscode_asp) | STM32MP257/A35ターゲット |
