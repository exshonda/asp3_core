...
| STM32 HAL統合 | STM32Cube HALと協調動作（第一目的の第3弾）。外側リポジトリ(asp3_stm32cubemx)で管理＝`ASP3_TARGET_DIR`。移植元が旧世代asp3(TECS/Ruby)のため非TECS+Python cfg化が要・toolchainはarm-none-eabi gcc・CubeMX生成依存（現行版に合わせる） | 中 |
