...
cmake --preset pico2-riscv -B build/pico2-riscv && cmake --build build/pico2-riscv

# STM32MP257F-DK (Cortex-A35 / 実機)
cmake --preset stm32mp257-a35 -B build/stm32mp257-a35 && cmake --build build/stm32mp257-a35
```
...
解析・検証：
```bash
./build/posix/asp 2>&1 | python3 scripts/parse_slog.py            # 人間可読
./build/posix/asp 2>&1 | python3 scripts/parse_slog.py --json \
  > actual.jsonl
