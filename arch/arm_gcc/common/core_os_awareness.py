# Core (ARM(A/R-profile) / GICv2) awareness helpers for gdb OS-awareness (ASP3).
#
# 役割: コア（arm 共通）依存の低レベル知識。arm64 版（arch/arm64_gcc/common/
#       core_os_awareness.py）と同じ GICv2 の知識を 32bit arm 用に提供する。
#       - GICv2 のレジスタ配置を知り，「指定 INTID の割込み許可/禁止状態」を GICD の
#         レジスタから読む処理を提供する。GICD のベースアドレスはチップ依存なので
#         引数で受け取る（chip_os_awareness が渡す）。
#       - arm 共通部の割込みハンドラテーブル _kernel_inh_table（INTNO で添字付けした
...
# GICv2 Distributor レジスタオフセット（arch/arm_gcc/common/gic_kernel_impl.h と一致）
...
def inh_handler(intno):
    ...
    arm 共通部の _kernel_inh_table[]（INTNO で添字付けした FP 配列）を読む。
...
def primap_bit(pri):
    ...
    arm は PRIMAP_BIT(pri)=(0x8000>>pri)（MSB 詰め，CLZ サーチ用。
    core_kernel_impl.h と一致させること）。
    """
    return 0x8000 >> int(pri)
