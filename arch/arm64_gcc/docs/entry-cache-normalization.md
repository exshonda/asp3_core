# エントリ時キャッシュ／MMU／GIC状態の正規化（ARM64）

リセットを介さずにカーネルを再起動した場合（デバッガによるイメージ再ロード，
ブートローダがキャッシュ有効のまま制御を渡す等）に，カーネル初期化が前提とする
クリーンな状態をカーネル自身が作り出すための正規化について述べる．対象は2つ:

- **キャッシュ／MMU 状態**: コア依存部が提供する `arm64_normalize_cache_state_el1`
  （`common/core_support.S`）．本書 §1〜§9．
- **GIC（割込みコントローラ）状態**: `common/gic_kernel_impl.c` の `gicd_initialize_common`
  ／`gicc_init` での残留 pending／active 割込みのクリア．本書 §10．

両者は同じ「リセット無し再起動」への対策であり，前者はコアのキャッシュ／変換状態，
後者は割込み分配器（Distributor）／CPUインタフェースの残留状態を正規化する．
...
§1〜§9 のキャッシュ／MMU 正規化と同じ「リセット無し再起動」への対策として，
GIC（Generic Interrupt Controller, GICv2）の残留割込み状態も正規化する．
実装は `common/gic_kernel_impl.c` の `gicd_initialize_common`（Distributor 側）と
`gicc_init`（CPU インタフェース側）にある．

### 10.1 必要性

割込みコントローラの状態は，コアのキャッシュ／MMU とは独立に，リセットを介さない
再起動（JTAG 再ロード等）をまたいで残存する．カーネル初期化は「pending／active な
割込みが無い」状態を暗黙の前提とするため，前回実行が残した状態でエントリすると
誤動作する．

### 10.2 失敗のメカニズム

1. **残留アクティブ割込み**: 割込みハンドラ実行中（IAR で ack 済み・EOI 前）に
   デバッガでハルトして再ロードすると，その割込みが GIC に **active** のまま残る．
   GIC は active 割込みの優先度（実行優先度＝RPR）以下の割込みをマスクするため，
   再起動後にタイマ割込み等が**一切配送されず**，スケジューラが起動しない／
   デッドロックする．
   - マルチコア構成（FMP3 等）では，カーネル終了 IPI（`ext_ker` の SGI）が
     `irc_end_int` を実行せず停止するため EOI されず active のまま残り，
     以降のプロセッサ間割込み（ディスパッチ要求 SGI 等）をマスクして
     MP 動作がデッドロックする．これが最も典型的な発現例である．
2. **残留ペンディング割込み**: ハルト中に発生した割込み（タイマ等）が pending の
   まま残り，再起動直後の不正なタイミングで配送される．
3. **banked 割込み（SGI 0-15／PPI 16-31）の見落とし**: 通常の Distributor 初期化は
   グローバル割込み（SPI，番号 32 以上）のみを対象とすることが多い．しかし
   **セキュア物理タイマは PPI（INTID 29）** であり banked 側にあるため，
   SPI のみのクリアでは残留する．

### 10.3 提供する機構

通常のリセット起動時は pending／active いずれも存在しないため，以下はいずれも
**実質的な副作用を持たない**（防御的初期化）．

- **`gicd_initialize_common`（Distributor）**: 既存の SPI ペンディングクリアに加えて，
  1. **アクティブ状態の全クリア**（`GICD_ICACTIVERn`，番号 0 から全割込み＝banked 含む）
  2. **banked 割込み（PPI/SGI, 0-31）のペンディングクリア**（`GICD_ICPENDRn[0]` と
     SGI 用 `GICD_CPENDSGIRn`）．セキュア物理タイマ PPI(29) の残留を排除する．
- **`gicc_init`（CPU インタフェース）**: CPU インタフェース有効化後に，
  IAR で ack→EOIR で完了，を spurious（INTID 1023）が返るまで繰り返す
  **ペンディングドレインループ**で，CPU インタフェースの実行優先度をアイドルへ戻す．
  （有効化前の IAR は `GICC_CTLR=0` では機能しないため，有効化後に行う．）

### 10.4 設計上の注意

- **アクティブのクリア手段**: GICv2 では Distributor の clear-active レジスタ
  （`GICD_ICACTIVERn`, オフセット 0x380）への書込みで active 状態をクリアできる．
  EOIR は「現在ハンドラが処理中の割込み」を完了させる用途であり，INTID が不明な
  残留 active を任意にクリアする用途には使えないため，Distributor 側で行う．
- **CPU インタフェースの実行優先度**: active のクリアに加え，CPU インタフェース側に
  残った実行優先度はドレインループ（IAR/EOIR）で排出する．IAR は一度の読出しで
  1 つを ack するため，読んだ値を保存して同じ値で EOIR する（二重読み出しは
  ack の取りこぼし＝EOI 不整合になるため不可）．
- **マルチコアでの送信元 CPU**: SGI の EOIR は送信元 CPU フィールド（bit[12:10]）を
  含むため，IPI を EOI して active を解除する実装（FMP3 等）では送信元 CPU 分を
  発行する．本 ASP3（単一コア）構成では IPI を用いないため，Distributor の
  clean-active で十分である．
- **適用範囲**: 本処理は `common/gic_kernel_impl.c` にあり，arm64 の全ターゲット
  （stm32mp257f_dk／zcu102 等）で常時有効．いずれも GICv2(GIC-400) であり共通に機能する．
  GICv3／4 経路（システムレジスタ ICC_*）への適用は別途必要．

### 10.5 再現方法

§8 のキャッシュ正規化と同様に，リセット無しでイメージを再ロードして再現する．
GIC 残留の典型は「割込みハンドラ実行中（あるいはマルチコアの終了 IPI 未 EOI 状態）
でのハルト→再ロード」で，対策なしでは再起動後にタイマ／IPI が配送されず無進行
（バナーは出るがタスクが走らない／MP がデッドロックする）となり，対策ありでは
正常に起動する．
