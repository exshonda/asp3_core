/*
 *		トレースログ用のサービスコール機能コード定義
 *
 *  【asp3_core追加】FMP3 3.3 の include/kernel_fncode.h（svc.defから
 *  gensvc.rbで生成されたもの）をトレースログ専用にリネームして取込んだ
 *  ファイル．ASP3には機能コードの定義が存在しない（ソフトウェア割込み
 *  によるサービスコール呼出しを持たない）ため，トレースログの
 *  LOG_*フック（arch/tracelog/trace_log.h）とデコード
 *  （arch/tracelog/trace_dump.c・trace_slog.c）の間でのみ使用する．
 *
 *  FMP3との差分管理を容易にするため，ASP3に存在しないサービスコール
 *  （マイグレーション系・スピンロック系等）の機能コードも残置している．
 */

#ifndef TOPPERS_TRACE_FNCODE_H
#define TOPPERS_TRACE_FNCODE_H
