...
/*
 *		トレースログ機能のヘッダファイル（非TECS版）
 *
 *  【asp3_core変更】FMP3 3.3 の arch/tracelog/trace_log.h をASP3向けに
 *  変換して取込んだもの（TECS版 tTraceLog.c の置換え．経緯は
 *  docs/dev/cli-target.md）．FMP3からの主な変更点：
 *    - kernel_fncode.h → trace_fncode.h（トレースログ専用の機能コード）
 *    - FMP3専用フック（マイグレーション系・スピンロック系・get_pid等）
 *      のLOG_*マクロを削除（trace_fncode.hの機能コードは残置）
 *    - LOG_GET_TST_ENTER/LEAVE を追加（ASP3 3.7のget_tst対応）
 *    - 構造化ログ出力モード（TRACE_SLOG・trace_slog.c）を追加
 *
 *  このインクルードファイルは，TOPPERS_ENABLE_TRACE定義時に
 *  kernel/kernel_impl.h からインクルードされる．また，トレースログ機能
 ...
 *  機能コードの読み込み
 */
#include "arch/tracelog/trace_fncode.h"
...
/*
 *  【asp3_core追加】構造化ログモード
 *  バッファに記録せず，記録時に T=<tick_us>,EV=<event>[,...] 形式で
 *  低レベル文字出力へ逐次出力する（AGENTS.md §8・scripts/parse_slog.py）．
 */
#define TRACE_SLOG			UINT_C(0x08)
...
/*
 *  【asp3_core追加】構造化ログ1件の出力（trace_slog.c）
 */
extern void	trace_slog_print(const TRACE *p_trace);
...
Inline void
trace_write_4(uint_t type, LOGPAR arg1, LOGPAR arg2, LOGPAR arg3,
			  LOGPAR arg4)
{
	TRACE	trace;
 ...
 *  ターゲット依存の低レベル文字出力を利用する場合には，target_fput_log
 *  を渡す．
 */
...
/* 【asp3_core追加】get_tst（ASP3 3.7） */
#define LOG_GET_TST_ENTER(tskid, p_tskstat) trace_3(LOG_TYPE_SVC|LOG_ENTER, TFN_GET_TST, tskid, p_tskstat)
#define LOG_GET_TST_LEAVE(ercd, p_tskstat) trace_3(LOG_TYPE_SVC|LOG_LEAVE, TFN_GET_TST, ercd, *p_tskstat)
...
#define LOG_SET_TIM_LEAVE(ercd) trace_2(LOG_TYPE_SVC|LOG_LEAVE, TFN_SET_TIM, ercd)
...
#define LOG_ADJ_TIM_LEAVE(ercd) trace_2(LOG_TYPE_SVC|LOG_LEAVE, TFN_ADJ_TIM, ercd)
