 ...
 *		トレースログ機能（非TECS版）
 *
 *  【asp3_core変更】FMP3 3.3 の arch/tracelog/trace_log.c をASP3向けに
 *  変換したもの（経緯は docs/dev/cli-target.md）．FMP3からの主な変更点：
 *    - pcb.h（プロセッサ管理ブロック）への依存とプロセッサIDの記録を削除
 *    - SIL_LOC_SPN/SIL_UNL_SPN（スピンロック）→ SIL_LOC_INT/SIL_UNL_INT
 *    - 構造化ログモード（TRACE_SLOG）を追加：バッファに記録せず，
 *      trace_slog.c の trace_slog_print() で逐次出力する
 */
...
trace_wri_log(TRACE *p_trace)
{
	...
	if (trace_mode != TRACE_STOP) {
		...
		/*
		 *  【asp3_core追加】構造化ログモード：バッファに記録せず逐次出力
		 */
		if ((trace_mode & TRACE_SLOG) != 0U) {
			trace_slog_print(p_trace);
			SIL_UNL_INT();
			return(E_OK);
		}
