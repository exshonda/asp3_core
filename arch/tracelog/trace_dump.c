 ...
 *		トレースログのダンプ（非TECS版）
 *
 *  【asp3_core変更】FMP3 3.3 の arch/tracelog/trace_dump.c をASP3向けに
 *  変換したもの（経緯は docs/dev/cli-target.md）．FMP3からの主な変更点：
 *    - プロセッサID欄の出力を削除（出力形式：[時刻]: メッセージ）
 *    - FMP3専用のログ種別（タスク／ハンドラのマイグレーション・タスク
 *      フェーズ）のデコードを削除．FMP3専用サービスコール（mact_tsk等）
 *      のデコード表は，FMP3との差分管理のため残置（ASP3では発生しない）
 *
 *  ASP3に存在しない機能コードの定義は trace_fncode.h を参照．
 */
