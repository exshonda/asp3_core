...
/*
 *  sil.h のターゲット依存部（ARM MPS2-AN386 用）
 *
...
/*
 *  プロセッサのインディアン定義（Cortex-M4，リトルエンディアン）
 */
 ...
 *  割込み優先度のビット幅
 *
 *  Cortex-M4（AN386）は __NVIC_PRIO_BITS = 3＝3 ビット（8 レベル）を実装する．
 *  QEMU は 8 ビットを実装するが，3 ビットで設定した割込み優先度は上位 3 ビット
 *  に配置されるため，QEMU・実機の双方で正しく動作する．
 */
#define TBITW_IPRI     3
 ...
 *  一般共通レジスタに対するビット操作関数
 *
 *  CMSDK/SSE-200 にはアトミックなビット操作機構（RP2350 のような
