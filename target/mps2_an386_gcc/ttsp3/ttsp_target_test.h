/*
 *  TTSP3 適合性テスト：ターゲット依存定義（mps2_an386_gcc / Cortex-M4）
 *
 *  asp3_core 側に置く TTSP3 ターゲットテスト資産（TTSP3 本体は不変＝
 *  読み取り専用とする方針のため・docs/dev/ttsp3-conformance.md）。
 *  TTSP3 library/ASP/target/lpc55s69evk_gcc（Cortex-M33/arm_m_gcc）を
...
/*
 *  割込み番号（interrupt モジュールは SKIP．主に staticAPI 系がコンパイル・
 *  cfg チェックで参照する）
 *
 *  AN386（num-irq=32）では UART0 の RX=IRQ0（INTNO 16）／TX=IRQ1（INTNO 17）を
 *  SIO ドライバが CFG_INT で登録済みである．TTSP3 の staticAPI/CRE_ISR 系は
 *  「CFG_INT が無い INTNO」に対する E_OBJ 検出を確認するため，SIO の登録線
 *  （16/17）と衝突しない予備の外部 IRQ を割り当てる：IRQ24〜29 → INTNO 40〜45．
 */
#define TTSP_INTNO_A   40
#define TTSP_INTNO_B   41
#define TTSP_INTNO_C   42
#define TTSP_INTNO_D   43
#define TTSP_INTNO_E   44
#define TTSP_INTNO_F   45
#define TTSP_INVALID_INTNO  168  /* ターゲット非サポートの割込み番号（> TMAX_INTNO=48） */
