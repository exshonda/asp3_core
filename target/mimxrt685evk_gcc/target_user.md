...
- デバッグコンソール: **Flexcomm0 USART0**（オンボードのデバッグプローブ〔LPC-Link2〕の VCOM に接続）
...
すべて実機（EVK-MIMXRT685）で確認（2026-06-12．テスト全件は J-Link 構成で実施し，書込み・ボードランナ・test_porting・OS Awareness は標準の LinkServer 構成でも再確認済み）：
...
| OS Awareness | `osdebug`（gdb-multiarch．LinkServer gdbserver --attach／J-Link GDB Server の両構成）で atask/stask/sem/cyc/intr（NVIC ena/pend 含む）を確認 |
