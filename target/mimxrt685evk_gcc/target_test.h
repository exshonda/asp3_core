...
/*
 * テストプログラムのターゲット依存定義（MIMXRT685-EVK用）
 */
 ...
 *
 *  クロックを供給していないCTIMER1のNVIC IRQ 11（→ INTNO 27）を
 *  ソフト割込み源として使う（クロック停止中のためハードウェアからは
 *  割込みが発生しない）．arm_m は ras_int / prb_int をサポートし，
 *  NVICのソフトpend（ISPR）で発生・ハンドラ入口で自動クリアされる
 *  ため intno1_clear() は空でよい．
 */
#define INTNO1			(IMXRT600_CTIMER1_IRQn + 16)	/* 27 */
