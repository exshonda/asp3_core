...
**(1) ビルドディレクトリの生成**
```bash
cmake --preset pico2-m33 -B build/pico2-m33
```
> ツールチェイン（arm-none-eabi）に PATH を通しておくこと．

**(2) ビルド**
```bash
cmake --build build/pico2-m33    # -> asp.elf を生成
```
...
```bash
ninja -C build/pico2-m33 run      # OpenOCD で書き込み（program verify reset）→ そのまま実行開始
ninja -C build/pico2-m33 console  # 別端末: シリアルコンソールを開く（出力確認・キー入力用）
```
...
用意している（`ninja -C build/pico2-m33 <ターゲット>` で実行）:
...
```bash
ninja -C build/pico2-m33 console    # 端末C: シリアルコンソールを開いておく（出力を見る）
ninja -C build/pico2-m33 run        # 単に動かすだけ（最も手軽．gdb 不要）
ninja -C build/pico2-m33 swd-debug  # 1 端末でソースデバッグ（break 設定後 continue）

# 2 端末に分けたい場合（OpenOCD を起動しっぱなしにして gdb を繰り返したいとき）
ninja -C build/pico2-m33 openocd    # 端末1: OpenOCD を起動して放置（gdb サーバ :3333）
ninja -C build/pico2-m33 gdb        # 端末2: gdb を起動（再ビルド後はこれを再実行するだけ）
```
...
- 上書き可能なキャッシュ変数: `OCD_IF`/`OCD_TGT`/`OCD_SPEED`，
  ...
  例: `cmake -B build/pico2-m33 -DTTY=/dev/ttyACM1`
