...
### カーネルタイマ方式
`Makefile.target` の `KERNEL_TIMER` で選択する（既定: `TIM`）．
...
- 必要ツール: クロスコンパイラ，Ruby（コンフィギュレータ用），OpenOCD（Raspberry Pi フォーク），
  シリアル端末（picocom 等）．

#### 1.2 クロスコンパイラ（arm-none-eabi-gcc）
```bash
# Arm GNU Toolchain（arm-none-eabi）を展開し PATH を通す（または configure.rb の -V で指定）
...
**(1) ビルドディレクトリの生成**
```bash
mkdir <OBJ>          # 例: <ASP3>/OBJ_pico2
cd <OBJ>
ruby <ASP3>/configure.rb -T raspberrypi_pico2_gcc \
     -V /usr/local/tools/arm-gnu-toolchain-14.3.rel1-x86_64-arm-none-eabi/bin
```
> ツールチェインに PATH を通している場合は `-V` は省略できる．
...
```bash
make run             # OpenOCD で書き込み（program verify reset）→ そのまま実行開始
make console         # 別端末: シリアルコンソールを開く（出力確認・キー入力用）
```
...
| コマンド | 動作 | OpenOCD |
|---|---|---|
| `make run`       | OpenOCD だけで書き込み＆実行（gdb 不要．最も手軽） | 完了で自動終了 |
...
- 上書き可能な変数: `ELF`（既定 `asp`），`OPENOCD`/`OCD_IF`/`OCD_TGT`/`OCD_SPEED`，
  `TTY`（既定 `/dev/ttyACM0`），`BAUD`（既定 `115200`）．
  例: `make console TTY=/dev/ttyACM1`

手動で同等のことを行う場合:
```bash
# 書き込みのみ（make run 相当）
openocd -f interface/cmsis-dap.cfg -f target/rp2350.cfg \
  -c "adapter speed 5000" -c "program asp verify reset exit"

# gdb デバッグ（OpenOCD 起動後に）
arm-none-eabi-gdb asp
...
- **Secure ステートと `ENABLE_TRUSTZONE`**: RP2350 の bootROM は ARM イメージを **Secure
  ステート**で起動する．このため `Makefile.target` で **`ENABLE_TRUSTZONE = 1`** とし，
  Secure ステート用の `EXC_RETURN`（`0xfffffffd`）を使用している．`0` にすると Non-secure 用の
...
- **BOOTSEL ブート（参考・未検証）**: IMAGE_DEF ブロックを含むため，`picotool load asp` →
  `picotool reboot`（BOOTSEL モード）でも書き込めると考えられるが未検証．動作確認済みの方法は
  OpenOCD（`make run`）である．
- **ランタイムテスト**: `test/testexec.rb` による標準テスト 36 件中 34 件 PASS を確認済み
  ...
  **cpuexc1 / cpuexc4 は FAIL するが既知の制限**（割込みロック PRIMASK=1 中の CPU 例外が
  ...
  依存部を使う全ターゲット共通）．
- **UART1**: `tSIOPortTarget.cdl` にプロトタイプ（`SIOPortTarget2`）を定義してあるが，GPIO の
  ファンクション設定は UART0（GP0/GP1）分しか行っていないため，使用する場合は
  `hardware_init_hook()` への追加が必要（未検証）．
