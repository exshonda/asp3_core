#
#		実機実行・デバッグ用CMakeターゲット（RaspberryPi Pico2用）
#
#  Makefile.targetのデバッグ系ターゲットのCMake版．aspターゲット定義後に
...
#
#  上書き可能なキャッシュ変数: OCD_IF / OCD_TGT / OCD_SPEED / TTY / BAUD
#

set(GDB_SCRIPT ${TARGETDIR}/swd-debug.gdb)

set(OCD_IF interface/cmsis-dap.cfg CACHE STRING "OpenOCD interface config")
set(OCD_TGT target/rp2350.cfg CACHE STRING "OpenOCD target config")
...
add_custom_target(swd-debug
    COMMAND bash -c "echo '[swd-debug] starting OpenOCD in background (log: openocd-bg.log)'; openocd -f ${OCD_IF} -f ${OCD_TGT} -c 'adapter speed ${OCD_SPEED}' > openocd-bg.log 2>&1 & OCD_PID=$!; trap 'kill $OCD_PID 2>/dev/null' EXIT INT TERM; sleep 2; arm-none-eabi-gdb $<TARGET_FILE:asp> -x ${GDB_SCRIPT}"
    DEPENDS asp
    USES_TERMINAL
    VERBATIM
