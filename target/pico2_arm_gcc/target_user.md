...
`pico2_arm_gcc` 依存部は，Raspberry Pi **Pico 2**（RP2350 の ARM/Cortex-M33 モード）を
サポートする．ASP3 はシングルプロセッサカーネルであり，**Core0 のみ**を使用する（Core1 は
bootROM 内で待機したままとなる）．動作モードは **Secure ステート**（ARMv8-M）である．

- ターゲット略称: `pico2_arm_gcc`
  （BOARD=`pico2_arm`, CHIP=`rp2350`, PRC=`arm_m`, TOOL=`gcc`）
...
**(2) ビルド**
```bash
cmake --build build/pico2_arm    # -> asp.elf を生成
```
...
```bash
ninja -C build/pico2_arm run      # OpenOCD で書き込み（program verify reset）→ そのまま実行開始
ninja -C build/pico2_arm console  # 別端末: シリアルコンソールを開く（出力確認・キー入力用）
```
...
OpenOCD と gdb を起動できる CMake ターゲット（`target/pico2_arm_gcc/run.cmake`）を
用意している（`ninja -C build/pico2_arm <ターゲット>` で実行）:
...
```bash
ninja -C build/pico2_arm console    # 端末C: シリアルコンソールを開いておく（出力を見る）
ninja -C build/pico2_arm run        # 単に動かすだけ（最も手軽．gdb 不要）
ninja -C build/pico2_arm swd-debug  # 1 端末でソースデバッグ（break 設定後 continue）

# 2 端末に分けたい場合（OpenOCD を起動しっぱなしにして gdb を繰り返したいとき）
ninja -C build/pico2_arm openocd    # 端末1: OpenOCD を起動して放置（gdb サーバ :3333）
ninja -C build/pico2_arm gdb        # 端末2: gdb を起動（再ビルド後はこれを再実行するだけ）
```
...
- 上書き可能なキャッシュ変数: `OCD_IF`/`OCD_TGT`/`OCD_SPEED`，
  ...
  例: `cmake -B build/pico2_arm -DTTY=/dev/ttyACM1`
