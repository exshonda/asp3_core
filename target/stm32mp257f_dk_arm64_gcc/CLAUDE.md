...
```bash
cmake --preset stm32mp257f_dk_arm64 -B build/stm32mp257f_dk_arm64   # 非TECS（TECSは廃止済み）
cmake --build build/stm32mp257f_dk_arm64     # -> asp.elf（コンパイラ警告ゼロであること）
ninja -C build/stm32mp257f_dk_arm64 swd-run  # 実機実行（FSBL/minimal_boot 入り SD で起動済みのこと）
ninja -C build/stm32mp257f_dk_arm64 console  # / osdebug / swd-debug / openocd / gdb
```
