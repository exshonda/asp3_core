...
- エンジン（`os_awareness.py`）は ASP3 のカーネルシンボル（`_kernel_*`）と
  ...
  （mps2-an505・xlnx-zcu102・xilinx-zynq-a9・microchip-icicle-kit）で検証済み
...
```bash
ninja -C build/mps2_an505-qemu osdebug   # QEMU(-s -S)＋gdb-multiarch を起動
...
| arm_m（NVIC） | `arm_m_gcc/common` | rp2350（パススルー） | mps2_an505（chip層なし）・pico2_arm |
...
- **mps2-an505**: Secure 動作のため NVIC は Secure バンクが見える（QEMU では問題なし）。
