...
#  の責任を負わない．
# 
#  $Id: testcfg.rb 1881 2026-05-18 03:52:07Z ertl-hiro $
# 
...
#  テスト結果のチェック
#
def CfgTest(test, testSpec, genFlag=false)
  objDir = test.tr("a-z_", "A-Z-")
  ...
  Dir.chdir(objDir) do
    ...
    # エラーメッセージのチェック
    if File.exist?("#{$usedSrcDir}/test_cfg/#{test}/error.txt")
      diffFile(test, "error.txt", objDir)
    elsif File.size("error.txt") > 0
      puts("cat error.txt")
  ...
  when "all"
    ...
    if CFG_TEST_SPEC.has_key?(param)
      if ($clean_flag)
        ...
        BuildTest(param, CFG_TEST_SPEC[param], true) unless $exec_only
        CfgTest(param, CFG_TEST_SPEC[param], true) unless $build_only
      end
    elsif PASS3_TEST_SPEC.has_key?(param)
      if ($clean_flag)
        CleanTest(param, PASS3_TEST_SPEC[param])
      else
        BuildTest(param, PASS3_TEST_SPEC[param], true) unless $exec_only
        CfgTest(param, PASS3_TEST_SPEC[param]) unless $build_only
      end
    elsif PASS2_TEST_SPEC.has_key?(param)
      if ($clean_flag)
        CleanTest(param, PASS2_TEST_SPEC[param])
      else
        BuildTest(param, PASS2_TEST_SPEC[param], true) unless $exec_only
        CfgTest(param, PASS2_TEST_SPEC[param]) unless $build_only
      end
    elsif PASS1_TEST_SPEC.has_key?(param)
      if ($clean_flag)
        CleanTest(param, PASS1_TEST_SPEC[param])
      else
        BuildTest(param, PASS1_TEST_SPEC[param], true) unless $exec_only
        CfgTest(param, PASS1_TEST_SPEC[param]) unless $build_only
      end
    else
